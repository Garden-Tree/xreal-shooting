...
namespace Unity.XR.XREAL.Samples
{
    ...
    public class BeamProController : MonoBehaviour
    {
        ...
        [Header("プロジェクタイル（飛翔体）設定")]
        [Tooltip("射撃時に生成する飛翔体のプレハブ（未指定の場合は球体が自動生成されます）")]
        [SerializeField]
        private GameObject m_ProjectilePrefab;

        [Tooltip("飛翔体の移動速度（m/s）")]
        [SerializeField]
        private float m_ProjectileSpeed = 30f;

        [Tooltip("弾頭（飛翔体）のカラー（自動生成時やマテリアルカラー上書きに対応したプレハブに適用されます）")]
        [SerializeField]
        private Color m_ProjectileColor = new Color(1f, 0.9f, 0.6f, 1f);

        [Tooltip("トレイル（軌跡）のカラーグラデーション（未設定の場合は弾頭カラーから透明へのフェードになります）")]
        [SerializeField]
        private Gradient m_TrailGradient;

        [Header("フローティングテキスト設定")]
        [Tooltip("的命中時のスコア（+100など）表示カラー")]
        [SerializeField]
        private Color m_HitTextColor = Color.green;

        [Tooltip("外れペナルティ（-10など）表示カラー")]
        [SerializeField]
        private Color m_MissTextColor = Color.red;

        /// <summary>命中時のテキストカラー</summary>
        public Color HitTextColor => m_HitTextColor;
        /// <summary>外れ時のテキストカラー</summary>
        public Color MissTextColor => m_MissTextColor;
        ...
        private void Shoot()
        {
            ...
            {
                projObj = Instantiate(m_ProjectilePrefab, m_PointerTransform.position, m_PointerTransform.rotation);
            }
            else
            {
                // フォールバック: 簡易的な球体を生成
                projObj = GameObject.CreatePrimitive(PrimitiveType.Sphere);
                projObj.name = "AutoGeneratedProjectile";
                projObj.transform.position = m_PointerTransform.position;
                projObj.transform.rotation = m_PointerTransform.rotation;
                projObj.transform.localScale = Vector3.one * 0.1f;

                // 物理判定と干渉しないようコライダーは削除（Projectile.csが独自で衝突判定を行うため）
                if (projObj.TryGetComponent<Collider>(out var col))
                {
                    Destroy(col);
                }
                ...
                {
                    if (projObj.TryGetComponent<Renderer>(out var r))
                    {
                        r.material = m_DynamicReticleMaterial;
                    }
                }

                // トレイルの描画用に TrailRenderer を動的に追加
                TrailRenderer trail = projObj.AddComponent<TrailRenderer>();
                trail.time = 0.2f;
            ...
            // Projectile スクリプトをアタッチしてパラメータを初期化
        ...
        /// <summary>
        /// 飛翔体が命中した際に呼び出され、コントローラーにバイブレーションを発生させます。
        /// </summary>
